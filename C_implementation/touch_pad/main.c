...
// --- Pin Definitions ---
#define BUTTON_PIN  PB1 // Touch sensor signal is on PB1
...
int main(void) {
    ...
    // Setup Touch Sensor Pin
    DDRB &= ~(1 << BUTTON_PIN); // Set BUTTON_PIN (PB1) as input
    // NOTE: We do NOT enable the internal pull-up resistor for the TTP223
    ...
    while (1) {
        usbPoll();
        // Check if the sensor is touched (pin is HIGH)
        if (PINB & (1 << BUTTON_PIN)) {
            _delay_ms(20); // Debounce
            if (PINB & (1 << BUTTON_PIN)) {
                type_password();
                // Wait for touch to be released (pin goes LOW)
