...
#include <usbdrv.h>               // USB driver library
#include <aes.h>                  // AES library for encryption


#define KEY_ENTER 40              // USB HID key code for Enter
#define KEY_S 5                   // USB HID key code for 'S'
#define KEY_T 20                  // USB HID key code for 'T'
#define KEY_U 24                  // USB HID key code for 'U'
#define KEY_D 7                   // USB HID key code for 'D'
#define KEY_Y 21                  // USB HID key code for 'Y'
...
void encryptPassword(uint8_t *password) {
    uint8_t key[16];
    ...
    AES128_ECB_encrypt(password, key);
}
...
int main(void) {
    // Encrypted password
    uint8_t encryptedPassword[16] = {KEY_S, KEY_T, KEY_U, KEY_D, KEY_Y};
    ...
    while (1) {
        ...
        if (!(PINB & (1 << PB0))) {
            ...
            encryptPassword(encryptedPassword);  // Encrypt the password
            for (int i = 0; i < 16; i++) {
